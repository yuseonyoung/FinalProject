...
/* 메일 조회div*/
let makeMailList = function(rslt) {
	console.log(rslt)
	let mailList =
		`
		<div class="check">
		<div class="checkRemove" value="${rslt.mailNo}">
         <div style = "max-height: 100px;" class="row border-bottom border-200 hover-actions-trigger hover-shadow py-2 px-1 mx-0 bg-light" >
                  ...
                  <div class="row">
                    <div class="col-md-4 col-xl-3 col-xxl-2 ps-md-0 mb-1 mb-md-0">
                      <div class="d-flex position-relative">
                        ...
                        <div class="flex-1 ms-2"><a class="stretched-link inbox-link" href="/mail/${rslt.mailNo}">${rslt.senNm}</a>
                        </div>
                      </div>
                    </div>
                    <div class="col"><a class="d-block inbox-link" href="/mail/${rslt.mailNo}"><span style="height: 40px; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">${rslt.mailTitle}</span></a>
                    </div>
                  </div>
                </div>
...
/* 받은메일 조회 */
function retrieveMailList() {
	console.log("9999997777777777");
	let baseUrl = `/mail/list2`;
	$.ajax({
		url: baseUrl,
		method: "GET",
		...
		success: function(resp) {
			...
			<div class="card-header">
				<div class="col d-flex align-items-center">
					...
					<h5 class="fs-0 px-3 pt-3 pb-2 mb-0 border-bottom border-200">받은 메일</h5>
				</div>
			</div>
			`;
			let pageLists = "";
			...
			if (dataList?.length > 0) {
				for (let i = 0; i < dataList.length; i++) {
					if (dataList[i].mailDel == 'N') {
						cnt += 1;
						mailLists += makeMailList(dataList[i]);
