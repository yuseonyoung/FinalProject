...
$(function() {
	...
	$('#purOrderForm').on('submit', function(e) {
		e.preventDefault();
		...
		$.ajax({
			...
			success: function(resp) {
				...
				if (purOrderList.length > 0) {
					...
					purOrderList.forEach(function(data) {
						let additionalItems = data.itemCount > 1 ? `외 ${data.itemCount - 1}건` : '';
						let row = $('<tr>');
						row.append('<td><a href="javascript:;" class="preqLink">' + data.PREQ_DATE + '</a></td>');
						row.append('<td>' + (data.PREQ_DUE_DATE ? data.PREQ_DUE_DATE : '') + '</td>');
						row.append('<td>' + data.ITEM_NM + additionalItems + '</td>');
						row.append('<td>' + data.itemQty + '</td>');
						row.append('<td>' + data.PREQ_STAT + '</td>');
						row.append('<td>' + (data.PREQ_NOTE ? data.PREQ_NOTE : '') + '</td>');
						row.append('<input type="hidden" class="preqCd" id="preqCd" value="' + data.PREQ_CD + '"/>');
				...
				let tableHead = ` <table class="table table-bordered fs--1 mb-0" id="purTable">
	                            <thead class="bg-200 text-900">
	                                <tr>
	                                    ...
	                                    <th>납기일자</th>
	                                    <td colspan="1">
	                                        <input type="date" class="iText" id="findPreqDueDate" name="preqDueDate" placeholder="납기일자" style="width:90%" max="${getCurrentDate()}" value="${purOrderList[0].PREQ_DUE_DATE}"/>
				...
				for (let i = 0; i < purOrderList.length; i++) {
					...
					tableBody += `
                                <tr class="purOrderRow">
                                    ...
                                    <td>
                                        <input type="number" readOnly class="iText boderNone" id="findReqItemQty" name="reqItemQty" style="width: 100%" value="${reqItemQty}"/>
