...
* ----------  ---------  -----------------
* 2023. 11. 8.       김도현      최초작성
* 2023. 11. 20.	     김도현	   담당자 및 품목 모달창 추가
	...
	<div class="card-body pt-0 ">
		<div class="tab-content">
			<div class="tab-pane preview-tab-pane active" role="tabpanel" aria-labelledby="tab-dom-bb1c70a1-6c4c-451a-80b8-972dfdd01aa8"
				...
				<c:url value="/sale" var="saleUrl" />
				<form action="${saleUrl}" id="saleForm2" method="post">
					...
					<table class="table table-bordered table fs--1 mb-0" id="dataTable">
						<thead class="bg-200 text-900">
							<tr>
								<th>일자</th>
								<td colspan="5">
									<input type="date" id="saleDate" name="saleDate" placeholder="일자" style="width: 170px;"/>
						...
						<tbody>
							...
							<tr class="new-row-template">
								<td><!-- 품목코드 -->
									<input type="text" id="findItemCd" name="saleItem[0].itemCd" class="iText clsItemCd" />
									<span id="saleItem.itemCd" class="error"></span> 
								</td>
								<td><!-- 품목명 -->
									<input type="text" id="findItemNm" name="saleItem[0].item.itemNm" class="iText clsItemNm" />
									<span id="saleItem.item.itemNm" class="error"></span> 
								</td>
								<td><!-- 수량 -->
									<input type="number" id="qty" name="saleItem[0].saleQty" class="iText clsSaleQty" style="width: 120px" />
									<span id="saleItem.saleQty" class="error"></span>
								</td>
								<td><!-- 단가 -->
									<input type="number" id="uprc" name="saleItem[0].saleUprc" class="iText clsSaleUprc" style="width: 120px" />
				...
				<button id="QuoteBtn" class="btn btn-primary">견적서</button>
			</div>
		</div>
	</div>
...
<!-- 견적서 검색 모달 -->
<div class="modal fade" id="findQteModal" tabindex="-1"aria-labelledby="findQteModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
			<!-- 모달 내용 -->
			<div class="modal-header">
				<h5 class="modal-title" id="findQteModalLabel">견적서</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal"
					aria-label="Close"></button>
			</div>
			<div class="modal-body">
				<!-- 모달 내용이 들어갈 자리 -->
				<table class="table table-bordered table fs--1 mb-0" id="findQteDataTable">
						<thead class="bg-200 text-900">
							<tr>
								<th>견적서코드</th>
								<th>일자</th>
						...
						<tbody class="list" id="qteTableBody">
...
document.getElementById('saleForm2').addEventListener('submit', function (event) {
        ...
        $("#addRowBtn").on("click", function () {
           ...
           // $("#dataTable tbody").append(newRow);
            ...
            $(".clsItemCd").each(function(idx){
            	$(this).attr("name","saleItem["+idx+"].itemCd");
            });
            $(".clsItemCd").last().val("");
            //품목명
            $(".clsItemNm").each(function(idx){
            	$(this).attr("name","saleItem["+idx+"].item.itemNm");
            });
            $(".clsItemNm").last().val("");
            //수량
            $(".clsSaleQty").each(function(idx){
            	$(this).attr("name","saleItem["+idx+"].saleQty");
            });
            $(".clsSaleQty").last().val("");
            //단가
            $(".clsSaleUprc").each(function(idx){
            	$(this).attr("name","saleItem["+idx+"].saleUprc");
            });
            $(".clsSaleUprc").last().val("");
